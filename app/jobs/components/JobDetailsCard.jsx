        ...
        <div className='w-[330px] md:w-[390px] h-fit md:p-[16px] p-[10px] border border-[#CFD4D1] flex flex-col justify-around relative rounded-xl'>
            ...
            <div className='flex justify-between items-center'>
                <div className='flex items-center gap-2'>
                    ...
                    <h4 className='font-thin'>{job.employer_name || 'Unknown Employer'}</h4>
                </div>

                {/* bookmark button */}
                <button
                    className='h-10 w-10 rounded-full hover:bg-slate-100 hover:scale-90 flex justify-center items-center transition-all duration-300 text-2xl'
            ...
            <div className='flex gap-2'>
                ...
                <h4 className='capitalize text-[#8C8F8E] text-sm'>
                    {job.job_city && job.job_state ? `${job.job_city}, ${job.job_state}` : 'Location N/A'}
                </h4>

                {/* employment type */}
                <div className='w-fit text-sm capitalize px-2 py-1 bg-[#E6FCF1] text-[#0C9653] rounded-full flex items-center justify-center'>
                    {job.job_employment_type.length > 9 ? `${job.job_employment_type.slice(0, 9)}...` : job.job_employment_type || 'N/A'}
                </div>
            </div>
            ...
            <div className='absolute bottom-[16px] right-[16px]'>
                <p className='text-sm'>{job.job_posted_at}</p>
            </div>
